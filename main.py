...
# Load your model
model_file_path = r"C:/Users/HP/Desktop/sihcode/best_model.h5"
model = tf.keras.models.load_model(model_file_path)

# Class to condition mapping
class_to_condition = {
    0: "Acne and Rosacea",
    1: "Atopic Dermatitis",
...
if st.session_state.logged_in:
    # Display the content based on the user type
            ...
            with c1:
                ...
                if uploaded_image is not None:
                    ...
                    if st.button("Diagnose"):
                        ...
                        st.success(f"The model predicts: {condition}")
            with c2:
                picture = c2.camera_input("Take a picture")
                if picture:
                    c2.image(picture)
                    image = preprocess_image(picture)
                    if st.button("Diagnose"):
                        prediction = model.predict(image)
                        predicted_class = np.argmax(prediction, axis=1)[0]
                        condition = class_to_condition[predicted_class]
    ...
    st.warning("You must log in to access other pages.")
    st.stop()
